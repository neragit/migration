...
const MetaBarsAPI: React.FC<Props> = ({ data }) => {
    ...
    useEffect(() => {
        ...
        const xMax = Math.max(
            d3.max(barsData, d => Math.max(d.residents, d.apiReachAvg, d.adsAvg)) ?? 0
        ) * 1.1;
        ...
        svg.selectAll('rect.residents')
            ...
            .attr('x', padding.left)
            .attr('y', d => (yScale(d.lang) ?? 0) + yScale.bandwidth() * 0.05)
        ...
        svg.selectAll('rect.apireach')
            ...
            .attr('class', 'apireach')
            .attr('x', padding.left)
            .attr('y', d => (yScale(d.lang) ?? 0) + yScale.bandwidth() * 0.35)
        ...
        svg.selectAll('line.whisker')
            ...
            .attr('y1', d => (yScale(d.lang) ?? 0) + yScale.bandwidth() * 0.575)
            .attr('y2', d => (yScale(d.lang) ?? 0) + yScale.bandwidth() * 0.575)
            .attr('stroke', '#1976D2')
            .attr('stroke-width', 1.5)
            .attr('opacity', 0.8);

        // Whiskers for MetaManager
        svg.selectAll('line.whisker-metamanager')
            ...
            .attr('y1', d => (yScale(d.lang) ?? 0) + yScale.bandwidth() * 0.825)
            .attr('y2', d => (yScale(d.lang) ?? 0) + yScale.bandwidth() * 0.825)
            .attr('stroke', '#63B3ED')
            .attr('stroke-width', 1.5)
            .attr('opacity', 0.8);
            ...
            <div ref={containerRef} style={{ display: 'flex', gap: '50px', flexDirection: 'row', alignItems: 'flex-start', flexWrap: 'wrap' }}>
                <div style={{ flex: '1 1 400px' }}>
                    <h3 style={{ paddingTop: 0, paddingBottom: '10px', paddingRight: '10px', fontSize: '14px', fontWeight: 'bold' }}>
                        Usporedba službenih podataka MUP-a s procjenom publike prema jeziku na temelju Meta Graph API i MetaManager
                    </h3>
                    <svg
                        ref={barsRef}
                        width={chartWidth}
